<#
    ...
    .PARAMETER Install_Log
    The path to the log for which installation information and errors will be written to
...
[CmdletBinding()]
param (
    [Parameter(Mandatory)][string]$install_log
)

if (-not (Test-Path $install_log))
{
    New-Item -ItemType File -Path $install_log
}
...
{
    Add-Content -Path $install_log -Value "Windows Debugging Tools not detected. Downloading and Installing WDK."
    ...
    Start-Process -FilePath ".\windsdksetup.exe" -ArgumentList "/features OptionId.WindowsDesktopDebuggers /quiet /log `"$install_log`"" -Wait
}
else
{
    # Tools version check
    ...
    {
        Add-Content -Path $install_log -Value "Windows Debugging Tools invalid version detected. Downloading and Installing WDK."
...
{
    Add-Content -Path $install_log -Value "Python 3.7+ not detected. Downloading and Installing Python."
    ...
    Start-Process -FilePath ".\python-3.11.4-amd64.exe" -ArgumentList "/quiet PrependPath=1 InstallAllUsers=1 Include_doc=0 Shortcuts=0 Include_tcltk=0 /log `".$install_log`"" -Wait
}
else
{
    # Tools version check
    ...
    {
        Add-Content -Path $install_log -Value "Incorrect Python version detected. Downloading and Installing Latest Python."
...
Start-Process -FilePath $pythonpath -ArgumentList "-m pip install -r .\requirements.txt --log $install_log" -Wait
...
{
    Add-Content -Path $install_log -Value "7-zip not detected. Downloading and Installing 7-zip."
...
Install-WindowsFeature -Name UpdateServices -LogPath $install_log
...
# check for odbc driver and if missing, download and install ODBC Driver for SQL Server here

$searchRegex = "ODBC Driver \d+ for SQL Server"
$driverArray = Get-OdbcDriver

ForEach ($driver in $driverArray) {
    if (-not ($driver.Name -match $searchRegex) -and ($driver.platform -match "64-bit")){
        # indicates that ODBC driver for SQL server is not currently installed. should download and install

        Add-Content -Path $install_log -Value "MS ODBC driver for connecting to WID not detected. Downloading and installing ODBC driver."
        $ODBCDownloadlink = "https://go.microsoft.com/fwlink/?linkid=2266640"
        Invoke-WebRequest -Uri $ODBCDownloadlink -OutFile ".\msodbcsql.msi"
        Start-Process -FilePath msiexec -ArgumentList "/i .\msodbcsql.msi /quiet /norestart"
    }
}